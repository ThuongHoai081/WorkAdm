 ...
 * ----- Copyright 2017------------
 */
public class ServiceSorOrder implements IService {
	private static	String 			filePath		= null; 
	private	static	String 			urlPath			= null; 
	//--------------------------------Service Definition----------------------------------
	public static final String SV_MODULE 				= "EC_V3".toLowerCase();

	public static final String SV_CLASS 				= "ServiceTaSorOrder".toLowerCase();	
	...
	public static TaSorOrder reqNew(TaAutUser user, JSONObject json) throws Exception {
		...
		if (ent != null) {
			...
			Integer typ01 = (Integer) ent.req(TaSorOrder.ATT_I_TYPE_01);
			if (typ01 != null) {
				if (canAuthorizeWithRightsOne(user, typ01, SOR_NEW) == false) return null;
			} else return null;
			//------------------------------------
		}
		...
		if (ent.req(TaSorOrder.ATT_I_MAT_VAL_01) == null) ent.reqSet(TaSorOrder.ATT_I_MAT_VAL_01, 1);
		...
		oCode 			= (String) ent.req(TaSorOrder.ATT_T_CODE_02);
		if ( oCode == null || oCode.length()==0) {
			ent.reqSet(TaSorOrder.ATT_T_CODE_02, code);
		}

		//----khi new thi stat bị hạn chế-----------------------
		Integer stat 	= ent.reqInt(TaSorOrder.ATT_I_STATUS);
		Integer typ01	= ent.reqInt(TaSorOrder.ATT_I_TYPE_01);
		if (stat.equals(TaSorOrder.STAT_STOCK_COMPLETED)||stat.equals(TaSorOrder.STAT_COMPLETED) )
			ent.reqSet(TaSorOrder.ATT_I_STATUS		, TaSorOrder.STAT_STOCK_ACCEPT);
		...
		Integer			dealId		= ToolData.reqInt(obj, "dealId"	, null);
		String 			dealCode 	= ToolData.reqStr(obj, "deal"	, null);
		...
		doOrderSysthesis(ent, details, deal, TaSorOrder.SOR_ORDER_ADD);
	...
	private void doLst(TaAutUser user, JSONObject json, HttpServletResponse response) throws Exception  {
			...
			Integer iTotalRecords 			= reqListDynCountTotal(manId, type01, type02, type03, stat, dt01_from, dt01_to, dt03_from, dt03_to, supId, wId);				
			Integer iTotalDisplayRecords 	= reqListDynCount(null, manId, type01, type02, type03, stat, dt01_from, dt01_to, dt03_from, dt03_to, supId, wId, code, cliPhone, cliId, uId);
	...
	private void doLstDyn(TaAutUser user, JSONObject json, HttpServletResponse response) throws Exception  {
		...
		Integer iTotalRecords 			= reqListDynCountTotal(manId, type01, type02, type03, stat, dt01_from, dt01_to, dt03_from, dt03_to, supId, wId);				
		Integer iTotalDisplayRecords 	= reqListDynCount(dataTableOption, manId, type01, type02, type03, stat, dt01_from, dt01_to, dt03_from, dt03_to, supId, wId, code, cliPhone, cliId, uId);
	...
	private static Integer reqListDynCountTotal(Integer manId, Integer typ01, Integer typ02, Integer typ03, Integer stat, String dt01_from, String dt01_to, String dt03_from, String dt03_to, Integer supId, Integer warehouseId) throws Exception {
		Integer result = ViSorOrder.reqCountInvOrder(null, typ01, typ02, null, stat, manId, null, null, null, null);
		return result;
	}
	private static Integer reqListDynCount(Object[] dataTableOption, Integer manId, Integer typ01, Integer typ02, Integer typ03, Integer stat, String dt01_from, String dt01_to, String dt03_from, String dt03_to, Integer supId, Integer warehouseId, String code, String cliPhone, Integer cliId, Integer uId) throws Exception {
	...
	private static List<ViSorOrder> reqList(
			...
			Integer idMan, Integer uId, Integer whId, Integer matId,		
